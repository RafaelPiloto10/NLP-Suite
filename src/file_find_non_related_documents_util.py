...
#            4. the baseline for similarity.
#            Below which value of similarity would you consider the file to be an intruder? Recommend around 0.15.

import sys
import GUI_util
import IO_libraries_util

if not IO_libraries_util.install_all_packages(GUI_util.window, "Find Non-related Documents",
...
# This function reads the social actor list from the same directory
...
def load_soc_actors():
    ...
    if not os.path.isfile(fName):
        print(
            "The file " + fileName + " could not be found. The routine expects a csv dictionary file "
                                     "'social-actor-list.csv' in a directory 'lib' expected to be a subdirectory of "
                                     "the directory where the concreteness_analysis.py script is stored.\n\n"
                                     "Please, check your lib directory and try again.")
        mb.showerror(title='File not found',
                     message='The routine expects a csv dictionary file "social-actor-list.csv" in a directory "lib" '
                             'expected to be a subdirectory of the directory where the concreteness_analysis.py script '
                             'is stored.\n\n'
                             'Please, check your lib directory and try again')
        sys.exit()
...
def get_article_soc_actors_NER(dir_path, soc_acts, nlp, keywords, num_doc):
    ...
    for file in my_files:
        ...
        for wordNER, pos in nlp.ner(fcontent):
            if pos in ('LOCATION', 'DATE', 'ORGANIZATION', 'PERSON'):
                ...
                if lemma_NER not in postag_seen:
                    if lemma_NER in keywords[fileName]:
                        ...
                        # Reduce the size that is needed to detect NER. Saves time
                    else:
...
def percent_belongs(one_doc, all_doc):
    ...
    for word, freq in one_doc.items():
        list1_sum_square += freq * freq
        if word in all_doc:
            numerator += one_doc[word] * all_doc[word]
    for word, freq in all_doc.items():
        list2_sum_square += freq * freq
    l1_sqrt = list1_sum_square ** 0.5
    l2_sqrt = list2_sum_square ** 0.5
...
def check(dir_path, soc_acts, nlp, intruder_list, id_list, has_intruder, similarityIndex_base, freq_intruder, num_doc,
          terminal_output, f):
    parts = dir_path.split(os.path.sep)
    id = parts[-2]
    # Keywords are a dictionary, with keys being the name of each article;
    # values being a set of all social actors (NNP, NNPS) and NERs.
    compare = {}
    ...
    # Structure: key is the filename, value is list of keywords that is from all the other files.
    for doc_name in keywords.keys():
    ...
    if keywords == {}:
        ...
        for doc in this_intrude:
            ...
            if repeat_star:
                ...
                print(id + "," + str(len(keywords.keys())) + "," + doc + "," + IO_csv_util.dressFilenameForCSVHyperlink(
                    dir_path) + "," + IO_csv_util.dressFilenameForCSVHyperlink(filePath) + "," + str(
                    similar[doc]) + ",,**************************************")
                repeat_star = False
            else:
                print(id + "," + str(len(keywords.keys())) + "," + doc + "," + IO_csv_util.dressFilenameForCSVHyperlink(
                    dir_path) + "," + IO_csv_util.dressFilenameForCSVHyperlink(filePath) + "," + str(similar[doc]))
    return intruder_list, id_list, has_intruder, freq_intruder, num_doc


def main(core_nlp_dir, input_dir, output_dir, open_output_files, create_excel_charts, similarity_index_base):
    ...
    outputFilename = IO_files_util.generate_output_file_name('', input_dir, output_dir, '.csv', 'SSR', 'INTRD', '', '',
                                                             '', False, True)
    ...
    sys.stdout = f
    print(
        "Group identifier (Folder name), Documents in group, Intruder document, Group path, Document path, Similarity "
        "index ( <" + str(
            similarity_index_base) + ")")
    actors = load_soc_actors()
    dirs = glob(input_dir + '/*/')
    nlp = StanfordCoreNLP(core_nlp_dir)
    num_folder = 0
    num_doc = 0
    for d in dirs:
        has_intruder = False
        sys.stdout = terminal_output
        print("Processing folder " + str(num_folder + 1) + "/" + str(len(dirs)) + "; Folder name: " +
              d.split(os.path.sep)[-2])
        sys.stdout = f
        intruder_list, id_list, has_intruder, freq_intruder, num_doc = check(d, actors, nlp, intruder_list, id_list,
                                                                             has_intruder, similarity_index_base,
                                                                             freq_intruder, num_doc, terminal_output, f)
        if has_intruder:
    ...
    f.close()
    ##
    outputFilename = IO_files_util.generate_output_file_name('', input_dir, output_dir, '.csv', 'SSR', 'Intrud', 'freq',
                                                             '', '', False, True)
    files_to_open.append(outputFilename)
    ...
    if len(intruder_list) <= 320:
        print(
            "Similarity index cutoff, Number of intruders, Percentage of intruders among all documents for all groups "
            "(folders), Number of intruded groups (folders), Percentage of intruded groups (folders), List of intruded "
            "groups (folders), List of intruded documents")
        print(str(similarity_index_base) + "," + str(freq_intruder) + "," + str(
            round((freq_intruder / num_doc) * 100, 2)) + "," + str(freq_intruded_folder) + "," + str(
            round((freq_intruded_folder / num_folder) * 100, 2)) + "," + "; ".join(
            d.strip() for d in id_list) + "," + "; ".join(d.strip() for d in intruder_list))
    elif len(intruder_list) <= 640:
        print(
            "Similarity index cutoff, Number of intruders, Percentage of intruders among all documents for all groups "
            "(folders), Number of intruded groups (folders), Percentage of intruded groups (folders), List of intruded "
            "groups (folders), List of intruded documents, Continued list of intruded documents")
        print(str(similarity_index_base) + "," + str(freq_intruder) + "," + str(
            round((freq_intruder / num_doc) * 100, 2)) + "," + str(freq_intruded_folder) + "," + str(
            round((freq_intruded_folder / num_folder) * 100, 2)) + "," + "; ".join(
            d.strip() for d in id_list) + "," + "; ".join(d.strip() for d in intruder_list[:320]) + "," + "; ".join(
            c for c in intruder_list[320:]))
    else:
        print(
            "Similarity index cutoff, Number of intruders, Percentage of intruders among all documents for all groups "
            "(folders),Number of intruded groups (folders), Percentage of intruded groups (folders), List of intruded "
            "groups (folders), List of intruded documents, Continued list of intruded documents, Continued list of "
            "intruded documents")
        print(str(similarity_index_base) + "," + str(freq_intruder) + "," + str(
            round((freq_intruder / num_doc) * 100, 2)) + "," + str(freq_intruded_folder) + "," + str(
            round((freq_intruded_folder / num_folder) * 100, 2)) + "," + "; ".join(
            d.strip() for d in id_list) + "," + "; ".join(d.strip() for d in intruder_list[:320]) + "," + "; ".join(
            c for c in intruder_list[320:640]) + "," + "; ".join(c for c in intruder_list[640:]))
    f_e.close()
    sys.stdout = terminal_output
