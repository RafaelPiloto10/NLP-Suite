...
def checkExcel_extension(output_file_name,hover_var):
...
def run_all(columns_to_be_plotted,inputFilename, outPath, csv_outputFilename, chart_type_list,chart_title, column_xAxis_label_var,column_yAxis_label_var, outputExtension,label1='',label2='',label3='',label4='',label5='', useTime=True,disable_suffix=False,  count_var=0, column_yAxis_field_list = [],reverse_column_position_for_series_label=False ,series_label_list=[], second_y_var=0,second_yAxis_label='',hover_info_column_list=[]):
    data_to_be_plotted = prepare_data_to_be_plotted(csv_outputFilename, columns_to_be_plotted,chart_type_list,numeric_values_of_column_given, count_var, column_yAxis_field_list)
    transform_list = []
    ...
    if isinstance(data_to_be_plotted[0], list):
        excel_outputFilename = IO_files_util.generate_output_file_name(inputFilename, outPath, outputExtension, label1,
                                                                       label2, label3, label4, label5, useTime,
                                                                       disable_suffix)
        chart_title = chart_title
        create_excel_chart(GUI_util.window, data_to_be_plotted, excel_outputFilename, chart_title, chart_type_list,
                           column_xAxis_label_var, column_yAxis_label_var, reverse_column_position_for_series_label,
                           series_label_list, second_y_var, second_yAxis_label, csv_outputFilename, hover_var,
        ...
        if transform_list!=None:
            excel_outputFilename = IO_files_util.generate_output_file_name(inputFilename, outPath, outputExtension, label1, label2, label3, label4, label5, useTime, disable_suffix)
            # chart_title = chart_title + "\n" + tempFilename
            chart_title = chart_title
            create_excel_chart(GUI_util.window,transform_list,excel_outputFilename,chart_title,chart_type_list,column_xAxis_label_var,column_yAxis_label_var,reverse_column_position_for_series_label,series_label_list, second_y_var,second_yAxis_label,csv_outputFilename,hover_var,hover_info_column_list)
        else:
            excel_outputFilename = ""
    excel_outputFilename = checkExcel_extension(excel_outputFilename,hover_var)
...
def get_data_to_be_plotted_with_counts(inputFilename,withHeader_var,headers,columns_to_be_plotted,column_yAxis_field_list,dataRange):
    data_to_be_plotted=[]
    data_to_be_plotted = compute_column_frequencies(columns_to_be_plotted, dataRange, headers,column_yAxis_field_list)
...
def get_data_to_be_plotted_NO_counts(inputFilename,withHeader_var,headers,columns_to_be_plotted,data):
    ...
    #     for i in range(len(x1)): # go through all the x axis
    #         for j in range(len(data)): # go through all the data
    #             if (data[j][y1[i]]=="N/A"):     #j = row #, y1[i] one of col #s
    #                 data[j][y1[i]] = 0
    #             data_to_be_plotted.append([ data[j][x1[i]], data[j][y1[i]] ]) # append the data we want
    #             azip = list(zip(*data_to_be_plotted)) # prepare for next step
    #             data_to_be_plotted = list(zip(azip[0], map(float, azip[1]))) # change the y axis data into float

    # data_to_be_plotted = [data_to_be_plotted [i:i + len(data)] for i in range(0, len(data_to_be_plotted), len(data)) ] # divide the whole data into n list by the length of data
    # if withHeader_var==1: # if the data has header
    #     for i in range(len(x1)):
    #         data_to_be_plotted[i].insert(0,[headers[x1[i]], headers[y1[i]]])
    # else:
    #     for i in range(len(x1)):
    #         data_to_be_plotted[i].insert(0,['Column_'+ str(int(columns_to_be_plotted[i][0]) + 1), 'Column_'+ str(int(columns_to_be_plotted[i][1]) + 1)])
    return data_to_be_plotted
...
def compute_column_frequencies(columns_to_be_plotted, data_list, headers,specific_column_value_list=[]):
    column_list=[]
    column_frequencies=[]
    column_stats=[]
...
# Prepare the data to be used in create_excel_chart with the format:
...
def prepare_data_to_be_plotted(inputFilename, selected_series, chart_type_list,  count_var=0, column_yAxis_field_list = []):
    ...
    if numeric_values_of_column_given == 0:
        columns_to_be_plotted=get_column_numbers_to_be_plotted(withHeader_var,headers,selected_series)
    else:
        columns_to_be_plotted = selected_series
    if count_var == 1:
...
#def create_excel_chart(window,chart_type_list[0],chartTitle,data_to_be_plotted,output_file_name,column_xAxis_label='',column_yAxis_label='', n=1,num_label=4, second_y_var=0,chart_type_list=[]):
# def create_excel_chart(window,data_to_be_plotted,output_file_name,chartTitle,chart_type_list,column_xAxis_label,column_yAxis_label,hover_info_column_list=[],second_y_var=0,second_yAxis_label=''):
# def create_excel_chart(window,data_to_be_plotted,output_file_name,chartTitle,chart_type_list,column_xAxis_label='',column_yAxis_label='',second_y_var=0,second_yAxis_label='',inputFilename='',hover_info_column_list=[]): 
...
def create_excel_chart(window,data_to_be_plotted,output_file_name,chartTitle,chart_type_list,column_xAxis_label='',column_yAxis_label='',reverse_column_position_for_series_label=False,series_label_list=[],second_y_var=0,second_yAxis_label='',inputFilename='',hover_info_column_list=[]):
    ...
    # ensure filename extension is correct for hover_over effects (xlxm) and no effects (xlsx)
    output_file_name = checkExcel_extension(output_file_name,hover_var)

    if hover_var == 1: # hover-over effects are invoked and the Excel filename extension MUST be xlsm
    ...
    try:
        ...
        mb.showwarning(title='Output file error', message="Could not write the Excel chart file " + output_file_name + "\n\nA file with the same name is already open. Please close the Excel file and try again!")
        errorFound=True
    return errorFound