...
def get_TIPS(selected_TIPS, lookup, menu_lb, dropdown_field):
    ...
    try:
        ...
        mb.showinfo(title='TIPS keyError',
                    message="There was an error in the TIPS dictionary lookup for \n\n" + selected_TIPS +
    ...
    if menu_lb['state'] != 'disabled':
        if os.path.isfile(os.path.join(GUI_IO_util.TIPSPath, lookup[selected_TIPS])):
...
def checkTIPSDir(menu_lb):
    ...
    if not os.path.isdir(GUI_IO_util.TIPSPath):
        mb.showinfo(title='TIPS Warning',
                    message='The script could not find a TIPS subdirectory.'
                            'TIPS should be stored in a subdirectory called TIPS where your python script is stored.')
