...
if not IO_libraries_util.install_all_packages(GUI_util.window, "Verb Analysis",
                                              ['csv', 'os', 'collections', 'tkinter']):
    sys.exit(0)
...
def verb_voice_data_preparation(data):
    try:
        ...
        mb.showinfo("ERROR",
                    "INPUT MUST BE THE MERGED CoNLL TABLE CONTAINING THE SENTENCE ID. Please use the merge option "
                    "when generating your CoNLL table in the StanfordCoreNLP.py routine. Program will exit.")
        return
...
def voice_output(voice_word_list, data_divided_sents):
    ...
    # voice = [i + [CoNLL_util.Sentence_searcher(data_divided_sents, i[documentID_position], i[sentenceID_position])]
    # for i in voice]  # get full sentence
...
def verb_voice_stats(input_filename, output_dir, data, data_divided_sents, open_output_files, create_excel_charts):
    filesToOpen = []  # Store all files that are to be opened once finished
    ...
    # output file names
    verb_file_name = IO_files_util.generate_output_file_name(input_filename, '', output_dir, '.csv', 'NVA',
                                                             'Verb Voice', 'list')
    verb_stats_file_name = IO_files_util.generate_output_file_name(input_filename, '', output_dir, '.csv', 'NVA',
                                                                   'Verb Voice', 'stats')
...
def verb_modality_stats(config_filename, input_filename, output_dir, data, data_divided_sents, openOutputFiles,
                        createExcelCharts):
...
def verb_tense_stats(input_filename, output_dir, data, data_divided_sents, open_output_files, create_excel_charts):
    global postag_counter
    filesToOpen = []  # Store all files that are to be opened once finished
